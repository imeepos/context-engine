...
- [x] Token 鉴权中间件（保护市场 API）

**应用市场接口：**
- [x] `GET /plugins` — 搜索/列表（关键词、标签、分类、排序）
- [x] `GET /plugins/:id` — 插件详情（元数据、版本列表、下载量、评价）
- [x] `POST /plugins` — 发布新插件（上传代码+元数据+schema）
- [x] `PUT /plugins/:id` — 更新插件信息
- [x] `POST /plugins/:id/versions` — 发布新版本
- [x] `POST /plugins/:id/install` — 记录安装（返回插件包）
- [x] `DELETE /plugins/:id/install` — 记录卸载
- [x] `GET /plugins/installed` — 当前 Agent 已安装列表
- [x] `GET /plugins/published` — 当前 Agent 已发布列表
- [x] `GET /plugins/updates` — 检查已安装插件的可用更新
- [x] `POST /plugins/:id/reviews` — 提交评价/反馈

**D1 数据模型（新增表）：**
- [x] `users` — 用户/Agent 账户（Better Auth 管理）
- [x] `plugins` — 插件元数据（name, description, author_id, tags, downloads, status）
- [x] `plugin_versions` — 版本历史（version, source_code, schema, changelog）
- [x] `plugin_installs` — 安装记录（plugin_id, user_id, installed_version）
- [x] `plugin_reviews` — 评价反馈（plugin_id, user_id, rating, feedback）

**基础设施：**
- [x] Better Auth 集成（或同类轻量认证方案）
- [x] 插件代码存储（D1 元数据+代码，大型插件后续可扩展 R2）
- [x] 版本号规范（semver）
- **Status:** pending
...
- [x] 市场首页 — 热门插件、最新发布、分类浏览（调用 GET /plugins）
- [x] 插件详情页 — 描述、版本、评价、安装量（调用 GET /plugins/:id）
- [x] 我的已安装 — 已安装列表+版本+可用更新（调用 GET /plugins/installed）
- [x] 我的已发布 — 自己发布的插件管理（调用 GET /plugins/published）

**市场工具（从页面提取，AI 通过 tool 操作）：**
- [x] search-plugins — 搜索插件（调用 GET /plugins）
- [x] install-plugin — 安装插件（调用 POST /plugins/:id/install → 本地注册 → 注入路由和工具）
- [x] uninstall-plugin — 卸载插件（清理本地 → 调用 DELETE /plugins/:id/install）
- [x] update-plugin — 更新插件（调用 POST /plugins/:id/install 最新版 → 本地替换）
- [x] publish-plugin — 发布插件（打包 → 调用 POST /plugins）
- [x] publish-version — 发布新版本（调用 POST /plugins/:id/versions）

**本地管理：**
- [x] 已安装插件的本地持久化（JSON 记录插件 ID、版本、安装时间）
- [x] 安装后插件注入：新路由 + 新工具注入到 Browser，refresh 后 AI 可用
- [x] 现有 PluginLoaderService / PluginRegistryService 适配市场流程
- **Status:** pending
