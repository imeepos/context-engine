...
export class UpdatePluginTool {
  ...
  @Tool({
    name: 'update-plugin',
    description: 'Update installed plugin to target or latest version'
  })
  async execute(
    @ToolArg({ zod: z.string().min(1).describe('Plugin ID'), paramName: 'id' })
    id: string,
    @ToolArg({ zod: z.string().optional().describe('Target version'), paramName: 'version' })
    version?: string
  ) {
