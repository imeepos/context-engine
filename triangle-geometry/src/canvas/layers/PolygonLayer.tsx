      ...
      {polygons.map((polygon) => {
        ...
        return (
          <PolygonShape
            ...
            onSelect={() => selectPolygon(polygon.id)}
            onUpdateVertex={updatePolygonVertex}
          />
        )
      })}
    </>
...
function PolygonShape({
  ...
  const points = polygon.vertices.flatMap((v: any) => [v.x, v.y])
  ...
  return (
    <Group
      ...
      onDragMove={(e) => {
        ...
        if (dx !== 0 || dy !== 0) {
          ...
          polygon.vertices.forEach((_: any, index: number) => {
            let newPos = {
            ...
            onUpdateVertex(polygon.id, index, newPos)
          })
        }
      }}
      onDragEnd={() => {
    ...
    >
      ...
      {/* 顶点 */}
      {polygon.vertices.map((vertex: any, index: number) => (
        <Circle
          ...
          onDragMove={(e) => {
            e.cancelBubble = true
            const node = e.target
            onUpdateVertex(polygon.id, index, { x: node.x(), y: node.y() })
          }}
        />
      ))}
      {/* 顶点标签 */}
      {polygon.labels.map((label: any, index: number) => {
